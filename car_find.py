...
def find_part(bot, update, user_data):
    ...
    if user_data['user_car'] == '%%':
            update.message.reply_text('Нужно что-то ввести после /find')
...
# если нет совпадений отвечаем фразой
        if number_of_car == 0:
            update.message.reply_text('Такого номера автомобиля нет в базе. Если это ваш автомобиль, попросите админа внести его в базу.')
            return True